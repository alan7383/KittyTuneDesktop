...
object TidalAudioProvider {
    ...
    fun isLiveManifestUri(value: String?): Boolean =
        value?.let { v ->
            val path = runCatching { URI(v).path }.getOrNull() ?: v
            val norm = path.replace('\\', '/')
            norm.contains("/tidal-temp/") && norm.endsWith(".mpd", ignoreCase = true)
        } == true
    ...
    private fun requestDirectFlacFromEndpoint(
        ...
        client.newCall(request).execute().use { response ->
            ...
            val manifestLooksFlac = manifestDeclaresFlac
            val isAtmosManifest =
                isAtmosRequest ||
            ...
            val streamUri = liveManifestFile?.absolutePath
    ...
    private fun parseManifest(
        ...
        val text = String(
            Base64.getMimeDecoder().decode(manifestB64.trim()),
            Charsets.UTF_8,
        ).trim()
