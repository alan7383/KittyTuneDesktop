package com.alananasss.kittytune.audio.providers.qobuz
...
object QobuzAudioProvider {
    ...
    fun resolverInstanceBases(customInstances: String? = null): List<String> {
        val configured = customInstances.orEmpty()
            .split('\n', '\r', ',', ';', '\t', ' ')
            .map { it.trim() }
        ...
        return if (configured.isNotEmpty()) configured else listOf(DEFAULT_INSTANCE.trimEnd('/'))
    }

    fun normalizeResolverRegion(countryCode: String): String {
