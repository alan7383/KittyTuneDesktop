 ...
 * times: the insert is `INSERT OR IGNORE` on a unique id (issue #33).
 *
 * **Reads are memoised against [revision].** Every aggregate is a pure function of the table, so a
 * repeat of the same question between two writes has one answer, and the statistics screen asks the
 * same questions every time it opens. Nothing is invalidated by a timer — only by a write — so the cache
 * cannot go stale, and a screen that was slow enough to look broken now opens instantly the second time.
 */
object ListeningStatsRepository {
    ...
    /**
     * Bumped whenever the table changes, by a listen here or a listen merged from another device.
     *
     ...
     * different every time it is asked. Unbounded, the map would hold an entry per recomposition between
     * two writes; forty is far more than the screens ask for at once.
     */
    private val cache = BoundedCache<String, Cached>(40)
    ...
    private suspend fun <T> memo(key: String, compute: suspend () -> T): T {
        ...
        cache[key]?.let { if (it.revision == current) return it.value as T }
        val value = compute()
        ...
        cache[key] = Cached(current, value)
        return value
    }

    /**
     * Records a finished listen, on this device and in the log the other devices read.
     *
     * @param furthestPositionMs how far playback reached, which is what completion is judged on.
     * @param blocking wait for the write instead of launching it. Only for the shutdown hook, which
     *   must not return before the row is on disk — the process is about to end (issue #33).
     */
    fun recordEvent(
        track: Track,
        eventType: String,
        ...
        if (blocking) {
            kotlinx.coroutines.runBlocking { write() }
        } else {
            scope.launch { write() }
        }
    }
    ...
    suspend fun getSnapshot(since: Long, until: Long? = null): StatsSnapshot =
        memo("snapshot:$since:$until") { dao.getStatsSnapshot(since, until) }
