 ...
 * about it is newest. That is a last-writer-wins register per track id, which is the one merge rule that
 * survives both devices editing the same track while neither can see the other: liking on the phone and
 * unliking on the desktop converges to whichever the user actually did last, on every device, with no
 * order of arrival mattering.
 *
