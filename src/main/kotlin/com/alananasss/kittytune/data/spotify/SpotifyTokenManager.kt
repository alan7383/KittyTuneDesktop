...
object SpotifyTokenManager {
    ...
    suspend fun getValidAccessToken(): String? = withContext(Dispatchers.IO) {
        mutex.withLock {
            ...
            if (bootstrapped != null) {
                ...
                Logger.d(TAG, "Successfully bootstrapped anonymous Spotify access token (expires in ${(tokenExpiryMs - now) / 1000}s)")
                return@withLock cachedToken
            }
            ...
            if (fallback != null) {
                ...
                Logger.d(TAG, "Successfully acquired fallback Spotify access token (expires in ${(tokenExpiryMs - now) / 1000}s)")
                return@withLock cachedToken
            }

            Logger.e(TAG, "Failed to obtain Spotify access token")
        ...
        try {
            ...
            client.newCall(request).execute().use { response ->
                if (!response.isSuccessful) {
                    Logger.w(TAG, "Embed bootstrap HTTP error: ${response.code}")
                    return null
                }
                val html = response.body?.string() ?: return null
                val matcher = nextDataRegex.matcher(html)
                if (!matcher.find()) {
                    Logger.w(TAG, "No __NEXT_DATA__ script found in embed HTML")
                    return null
                }
            ...
            Logger.w(TAG, "Exception during embed bootstrap: ${e.message}")
        }
        return null
    }
    ...
    private fun bootstrapFromFallback(): Pair<String, Long>? {
        try {
            ...
            Logger.w(TAG, "Exception during fallback token bootstrap: ${e.message}")
        }
        return null
    }
}
