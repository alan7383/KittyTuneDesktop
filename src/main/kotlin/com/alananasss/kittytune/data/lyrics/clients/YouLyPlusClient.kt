...
import com.alananasss.kittytune.data.lyrics.models.YouLyPlusLine
...
object YouLyPlusClient {
    ...
    private val client by lazy {
        HttpClient(OkHttp) {
            ...
            defaultRequest {
                ...
                headers.append("User-Agent", "KittyTune")
            }
        ...
        return try {
            val lyrics =
                fetchTtml(cleanTitle, cleanArtist, cleanAlbum, durationSeconds)
                    ...
                    ?: throw IllegalStateException("Lyrics unavailable from YouLyPlus")
    ...
    private suspend fun fetchTtml(
        ...
        fetchFromMirrors(TTML_PATH, title, artist, album, durationSeconds) { body ->
            ...
            when {
                ...
                else -> runCatching { jsonFormat.decodeFromString<YouLyPlusTtmlResponse>(body).ttml?.trim() }.getOrNull()
    ...
    private suspend fun fetchLyricsAsLrc(
        ...
        fetchFromMirrors(LYRICS_PATH, title, artist, album, durationSeconds) { body ->
            val response = runCatching { jsonFormat.decodeFromString<YouLyPlusLyricsResponse>(body) }.getOrNull()
